...
class NodeMap extends Matrix {
  ...
  */
        ...
        OpenList.add(start);
              ...
              for(int i=0;i<Current.neighbours.size();i++)
          {
            ...
            {  
               ...
               Gprime=calculateCost(Current,Current.neighbours.get(i));  
              if(GetNeighbours(OpenList,Current.neighbours.get(i))==false)               
              {  
                OpenList.add(Current.neighbours.get(i)); 
                  
              }else if(Gprime>=Current.neighbours.get(i).getG())                  
              {  
                continue;
              } 
              Current.neighbours.get(i).parent=Current; 
              Current.neighbours.get(i).G=Gprime; 
            }  
  ...
  boolean GetNeighbours(ArrayList <Node> OpenList,Node Neighbours)
  
  {
    for(int a=0;a<OpenList.size();a++)
    
    {
      if(OpenList.get(a)==Neighbours)
  ...
  */
  private int calculateCost(Node nodeA, Node nodeB) {
    ...
    GPrime=nodeA.getG()+nodeB.movementCost;
    return GPrime;
